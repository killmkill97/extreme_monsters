...
@EventBusSubscriber
public class BarragePDMProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if (entity.getPersistentData().getBoolean("barragePDMb") == true) {
					...
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = new Object() {
							public Projectile getArrow(Level level, Entity shooter, float damage, int knockback, byte piercing) {
								AbstractArrow entityToSpawn = new PerfectDarkMatterMainEntity(ExtremeMonstersModEntities.PERFECT_DARK_MATTER_MAIN.get(), level) {
									@Override
			...
			entity.getPersistentData().putDouble("barragePDM", (entity.getPersistentData().getDouble("barragePDM") + 1));
			if (entity.getPersistentData().getDouble("barragePDM") >= 20) {
				entity.getPersistentData().putBoolean("barragePDMb", false);
			}
		}
	}
}
