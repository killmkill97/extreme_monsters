...
public class TrackerMeteorExplodeProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity immediatesourceentity) {
			...
			immediatesourceentity.discard();
		{
			...
			for (Entity entityiterator : _entfound) {
				if ((entityiterator instanceof Player || entityiterator instanceof Mob) && !(entityiterator instanceof HyperTrackerEntity) && !(entityiterator instanceof FlyingTrackerEntity) && !(entityiterator instanceof TrackerEntity)) {
					...
					if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
						_entity.addEffect(new MobEffectInstance(ExtremeMonstersModMobEffects.EXPLOSIVE, 300, 2));
					if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
						_entity.addEffect(new MobEffectInstance(ExtremeMonstersModMobEffects.BLACK_HOLE, 150, 1));
					if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
						_entity.addEffect(new MobEffectInstance(ExtremeMonstersModMobEffects.MELT, 60, 0));
				}
			}
		}
