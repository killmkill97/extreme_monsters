...
@EventBusSubscriber
public class HyperMagmaCubesteamProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity instanceof MagmaCube && (entity instanceof LivingEntity _livingEntity1 && _livingEntity1.getAttributes().hasAttribute(ExtremeMonstersModAttributes.HYPER_MOBS)
				...
				: 0) == 1) {
			if (entity.getPersistentData().getDouble("steam") <= 0) {
				entity.getPersistentData().putDouble("steam", 500);
			}
			if (entity.getPersistentData().getDouble("steam") >= 0) {
				entity.getPersistentData().putDouble("steam", (entity.getPersistentData().getDouble("steam") - 1));
			}
			if (entity.getPersistentData().getDouble("steam") == 160) {
			...
			if (entity.getPersistentData().getDouble("steam") <= 100) {
				if (world instanceof ServerLevel _level)
					_level.sendParticles(ParticleTypes.CLOUD, x, y, z, 5, 3, 3, 3, 0.8);
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("block.lava.extinguish")), SoundSource.MASTER, 1, (float) 0.8);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("block.lava.extinguish")), SoundSource.MASTER, 1, (float) 0.8, false);
					}
				}
				{
					...
					for (Entity entityiterator : _entfound) {
						if (!(entityiterator instanceof MagmaCube)) {
							if (entityiterator instanceof Mob) {
								entityiterator.hurt(new DamageSource(world.holderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("extreme_monsters:steam_damage")))), 1);
