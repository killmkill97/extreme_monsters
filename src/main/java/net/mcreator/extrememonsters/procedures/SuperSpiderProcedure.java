...
@EventBusSubscriber
public class SuperSpiderProcedure {
	@SubscribeEvent
			...
			if (ExtremeMonstersModVariables.MapVariables.get(world).Extreme) {
				if (entity instanceof Spider) {
					if (Mth.nextInt(RandomSource.create(), 1, 2) == 1) {
						...
						if (Mth.nextInt(RandomSource.create(), 1, 2) == 1) {
							if (entity instanceof LivingEntity _livingEntity7 && _livingEntity7.getAttributes().hasAttribute(ExtremeMonstersModAttributes.HYPER_MOBS))
								_livingEntity7.getAttribute(ExtremeMonstersModAttributes.HYPER_MOBS).setBaseValue(1);
