...
public class MobSeduceWorkProcedure {
				...
				}
				{
					...
					for (Entity entityiterator : _entfound) {
						if (entityiterator instanceof Monster) {
							if (!(entityiterator instanceof FlyingTrackerEntity)) {
								if (!(entityiterator instanceof TrackerEntity)) {
						...
						} else if (entityiterator instanceof Slime) {
							if ((entityiterator instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null) {
								if (entityiterator instanceof Mob _entity)
									_entity.getNavigation().moveTo(x, y, z, 1.4);
							}
