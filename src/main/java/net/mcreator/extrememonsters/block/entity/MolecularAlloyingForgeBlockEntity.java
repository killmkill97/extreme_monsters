...
public class MolecularAlloyingForgeBlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer {
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final SidedInvWrapper handler = new SidedInvWrapper(this, null);

	public MolecularAlloyingForgeBlockEntity(BlockPos position, BlockState state) {
		super(ExtremeMonstersModBlockEntities.MOLECULAR_ALLOYING_FORGE.get(), position, state);
	}
	...
	public Component getDefaultName() {
		return Component.literal("molecular_alloying_forge");
	}
	...
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new MolecularAlloyingForgeGUIMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
