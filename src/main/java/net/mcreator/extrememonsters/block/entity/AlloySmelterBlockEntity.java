...
public class AlloySmelterBlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer {
	// 8칸 예시, 슬롯 역할을 보고 플래그를 설계
	// 예: 0~5,7 = 입력 전용 / 6 = 출력 전용
	private final EMItemHandler inventory = new EMItemHandler(
	    9, //원하는 슬롯수
	    new boolean[]{ true, true, true, true, true, true, false, true, false },      // canInsert
	    new boolean[]{ false, false, false, false, false, false, true, false, false } // canExtract
	);
	public AlloySmelterBlockEntity(BlockPos position, BlockState state) {
		super(ExtremeMonstersModBlockEntities.ALLOY_SMELTER.get(), position, state);
	}
	...
	public Component getDefaultName() {
		return Component.literal("alloy_smelter");
	}
	...
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new AlloySmelterGUIMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}

	@Override
	public Component getDisplayName() {
		return Component.literal("§dAlloy Smelter");
	}
	...
	private final FluidTank fluidTank = new FluidTank(10000, fs -> {
		if (fs.getFluid() == ExtremeMonstersModFluids.SUPER_LAVA.get())
			return true;
		return false;
	}) {
		@Override
